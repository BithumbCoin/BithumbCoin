                                                                 ...
                                                                 const vector<permission_level>& auths )const
{
   ...
   if( !linked_permission_name ) // if action is linked to Bthbioio.any permission
                                                                   ...
                                                                   const vector<permission_level>& auths )const
{
   ...
   if( *unlinked_permission_name == config::Bthbioio_any_name )
      return fc::microseconds(0);
   ...
   for( const auto& act : actions ) {
      ...
      for( const auto& declared_auth : act.authorization ) {
         ...
         if( !special_case ) {
            ...
            if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is Bthbioio.any
...
#warning TODO: this comment sounds like it is expecting a check ("may") somewhere else, but I have not found anything else
   ...
   try {
      ...
      if( *linked_permission == config::Bthbioio_any_name )
...
void chain_controller::require_scope( const scope_name& scope )const {
   switch( uint64_t(scope) ) {
      case config::Bthbioio_all_scope:
      case config::Bthbioio_auth_scope:
