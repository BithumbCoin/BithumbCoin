#include <BithumbCoinio/abi_generator/abi_generator.hpp>
...
void abi_generator::handle_decl(const Decl* decl) { try {
  ...
  // If BithumbCoinIO_ABI macro was found, check if the current declaration
  ...
  // The current Decl was the type referenced in BithumbCoinIO_ABI macro
  if( type_has_actions ) return;

  // The current Decl was not the type referenced in BithumbCoinIO_ABI macro
  ...
  // If BithumbCoinIO_ABI macro was found, we will only check if the current Decl
